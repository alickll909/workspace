...
> 以下为本次变更涉及的前端 UI 审美与交互评分结果。详细评分报告见 `docs/test/ui-review/` 目录。

### UI 变更点评分总览

| 评分项 | 变更描述 | 审美分 | 交互分 | 总分 | 判定 |
|:------:|:---------|:------:|:------:|:----:|:----:|
| UI-001 | {变更描述} | {X}/5 | {X}/5 | {X}/10 | ✅ 及格 / ❌ 不及格 |
| UI-002 | {变更描述} | {X}/5 | {X}/5 | {X}/10 | ✅ 及格 / ❌ 不及格 |

### 评分汇总

| 总评分项 | 及格数 | 不及格数 | 结论 |
|:--------:|:------:|:--------:|:----:|
| UI 变更点评分 | {N} | {N} | ✅ 全部通过 / ⚠️ 部分打回 |

### 打回日志

| 打回编号 | UI评分项 | 总分 | 打回原因摘要 | 接收方 |
|:--------:|:--------:|:----:|:------------|:------:|
| REJECT-001 | UI-001 | {X}/10 | {简要扣分原因} | senior-game-pd |

> 打回的 UI 变更点需等待 senior-game-pd 重新交付后，再补充测试。
...
如果当前变更涉及**前端 UI 调整**（包括但不限于：页面布局修改、组件样式变更、交互流程调整、动效添加、色彩/字体/图标变更、表单/弹窗/导航等 UI 元素的新增或修改），必须在功能测试之外，对**每一个 UI 变更点**进行独立的审美与交互体验评分。

### 评分流程

1. **识别 UI 变更点**：通过 `git diff` 等分析变更文件，识别所有前端 UI 相关变更，每个独立的 UI 变更点均需评分
2. **截图/录屏采集**：使用测试工具（Playwright、auto-feedback、simdrive、mcp-android-emulator 等）对每个 UI 变更点截图或录屏，作为评分依据
3. **逐项评分**：对每个 UI 变更点，按以下评分标准打出 1-10 分
...
| 分值 | 评价 | 标准 |
|:----:|:----:|------|
| 5 | 卓越 | 视觉层次清晰、色彩搭配和谐、元素对齐精准、间距合理、与整体设计语言一致，具有专业感 |
| 4 | 良好 | 整体美观，有少量可优化细节但不影响用户体验 |
| 3 | 合格 | 基本可用，但存在明显审美缺陷（如色彩不协调、间距不一致、对齐偏差） |
| 1-2 | 不及格 | 视觉混乱、元素错位、色彩冲突、明显缺乏设计考量 |
| 0 | 不可接受 | 布局严重错乱、元素重叠、文本不可读、视觉根本无法使用 |

#### 交互维度（0-5 分）

| 分值 | 评价 | 标准 |
|:----:|:----:|------|
| 5 | 卓越 | 交互流畅、反馈及时、操作符合用户预期、手势/点击区域大小合理、无障碍考虑到位 |
| 4 | 良好 | 交互整体流畅，有少量可优化细节 |
| 3 | 合格 | 交互可用但存在明显可优化点（如缺乏加载状态、点击反馈不足、操作路径过长） |
| 1-2 | 不及格 | 交互卡顿、反馈缺失、操作逻辑混乱、用户容易误操作 |
| 0 | 不可接受 | 交互无法正常完成功能、核心操作不可达 |

### 总分判定

| 总分 | 判定 | 处理方式 |
|:----:|:----:|:---------|
| **≥ 8 分** | 及格 | 评分通过，记录到测试报告即可 |
| **< 8 分** | **不及格** | **必须打回给 senior-game-pd（产品经理）重新设计**，同时在测试报告中标记为 BLOCKED，具体流程：<br>1. 生成详细评分报告，逐项说明扣分原因和改进建议<br>2. 将评分报告发送给 senior-game-pd<br>3. 在测试报告中将该 UI 变更点标记为 `❌ UI评分未达标`<br>4. 暂停该模块的后续测试，待产品侧重新交付后再继续 |

### 评分报告格式

每个 UI 变更点的评分报告输出到 `docs/test/ui-review/` 目录，命名格式：`{模块名称}_ui_review_{YYYYMMDD}.md`，内容如下：
...
| 编号 | 变更描述 | 涉及文件 | 审美分 | 交互分 | 总分 | 判定 |
|:----:|:---------|:--------:|:------:|:------:|:----:|:----:|
| UI-001 | {描述} | {文件路径} | {X}/5 | {X}/5 | {X}/10 | 及格/不及格 |
| UI-002 | {描述} | {文件路径} | {X}/5 | {X}/5 | {X}/10 | 及格/不及格 |
...
| 打回编号 | 打回时间 | 打回原因 | 发送方 | 接收方 |
|:--------:|:--------:|:--------:|:------:|:------:|
| REJECT-001 | {时间} | {原因} | senior-game-tester | senior-game-pd |
```

> **注意**：UI 评分不是可选项。只要 `git diff` 检测到前端 UI 变更文件（`.vue`、`.jsx`、`.tsx`、`.css`、`.scss`、`.less`、`.html`、`.svg`、动画/过渡配置文件等），**必须**执行评分流程。评分报告与测试报告同等重要，必须输出到文档中。
